...
.. code-block:: cpp
    ...
    */
    ...
    {
        //注意拿到的不是一个数组，而是一个十六进制的枚举，如果想判断该属性是否包含某个修饰词，要用与运算
        ObjCPropertyAttribute::Kind attribute = decl->getPropertyAttributes();
        bool isCopy = (attribute & ObjCPropertyAttribute::Kind::kind_copy) > 0;
        string type = decl->getType().getAsString();
        vector<string> blackList = vector<string>{"NSString", "NSAttributedString", "NSArray", "NSDictionary", "NSURLRequest"};
        for (auto typeName : blackList) {
            if (type.find(typeName)!=type.npos && !isCopy) {
