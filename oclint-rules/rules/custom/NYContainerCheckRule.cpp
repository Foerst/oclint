...
class NYContainerCheckRule : public AbstractASTVisitorRule<NYContainerCheckRule>
{
public:
...
.. code-block:: cpp
    ...
    */
    ...
    {
        int count = node->getNumElements();
        for (int i=0; i<count; i++) {
            ObjCDictionaryElement element = node->getKeyValueElement(i);
        
            checkIfBranch(element.Value, node->getBeginLoc(), true);
        }
...
//        ObjCMessageExpr::ReceiverKind kind = node->getReceiverKind();
        if (node->isInstanceMessage()) {
            string receiverType = string(node->getReceiverInterface()->getName());
            string msgName = node->getMethodDecl()->getNameAsString();
            if (receiverType.compare("NSMutableString") == 0) {
                if (msgName.compare("appendString:") == 0) {
                    checkIfBranch(node->getArg(0), node->getBeginLoc(), false);
                }
            } else if (receiverType.compare("NSMutableArray") == 0) {
                if (msgName.compare("addObject:") == 0) {
                    checkIfBranch(node->getArg(0), node->getBeginLoc(), true);
                }
            } else if (receiverType.compare("NSMutableDictionary") == 0) {
                if (msgName.compare("setObject:forKey:") == 0) {
                    checkIfBranch(node->getArg(0), node->getBeginLoc(), true);
                }
            }
        }
    ...
    void checkIfBranch(Expr *expr, SourceLocation endLoc, bool isArray) {
        
        /*
         string argName = string(expr->getStmtClassName());
         [str appendString:@""] => ObjCStringLiteral
         NSString *arg = nil;
         [str appendString:arg] => ImplicitCastExpr
         */
        
        ImplicitCastExpr *implicitCastExpr = dyn_cast_or_null<ImplicitCastExpr>(expr->getExprStmt());
;
        if (implicitCastExpr) {
            string castName = string(implicitCastExpr->getCastKindName());
            if (castName.compare("NullToPointer")==0) {
                addViolation(implicitCastExpr, this, "参数请确保不为nil!");
                return;
            }
            
            if (isArray) {// array 再嵌套一层 ImplicitCastExpr
                implicitCastExpr = dyn_cast_or_null<ImplicitCastExpr>(implicitCastExpr->getSubExpr());
                if (!implicitCastExpr) {
                    return;
                }
            }
            
            DeclRefExpr *refExpr = dyn_cast_or_null<DeclRefExpr>(implicitCastExpr->getSubExpr());
            if (refExpr) {
                string name = refExpr->getDecl()->getNameAsString();
                
                VarDecl *varDecl = dyn_cast_or_null<VarDecl>(refExpr->getDecl()->getCanonicalDecl());
                
                if (varDecl) {
                    if (varDecl->getInit() == nullptr) {
                        addViolation(refExpr, this, name +"未初始化");
                    } else {
                        //参数定义到当前参数名之间的代码
                        SourceLocation startLocation = varDecl->getBeginLoc();
                        SourceLocation endLocation = endLoc;
                        ...
                        if (length > 0) {// 中间其他语句if
                            ...
                            blockStatementString = std::regex_replace(blockStatementString, pattern, "");
                        
                            string keyword = ".*if.*" + name + ".length";
                            
                            regex find(keyword + "\\s*>?\\s*0?.*");
                            
                            if (!regex_match(blockStatementString, find)) {
                                addViolation(refExpr, this, "参数:"+name+"请确保不为nil; 或者用stringParam()包裹");
                            }
                        } else {
                            addViolation(refExpr, this, "参数:"+name+"请确保不为nil; 或者用stringParam()包裹");
                        }
...
static RuleSet rules(new NYContainerCheckRule());
