...
def lint(out_file):
        lint_command = '''oclint-json-compilation-database -e Pods -e Amr -e bzip2-1.0.6 -e bsdiff-4.3 -e alihttpdns -e DerivedData -e CCBFaceRecoSDK -e YJYZFoundation.framework \
        -- \
        --verbose \
        ...
        -max-priority-1 100000 -max-priority-2 100000 -max-priority-3 100000 \
        --report-type pmd \
        >> %s''' % (CYCLOMATIC_COMPLEXITY, LONG_CLASS, LONG_LINE, LONG_METHOD, MINIMUM_CASES_IN_SWITCH, NPATH_COMPLEXITY, NCSS_METHOD, NESTED_BLOCK_DEPTH, out_file)
